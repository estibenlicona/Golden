...
<div class="container pb-5">
	...
	<div class="row">
		<div class="col-md-6">
			...
			<div class="card d-flex flex-row bg-dark text-light p-4 mt-3">
				<div style="width: 50%">
					...
					<h6 class="mb-1">Shirt Name:</h6>
					<h6 class="mb-1">Nationality:</h6>
				...
				<div style="width: 50%" class="d-flex flex-column align-items-end">
					...
					<h6 class="mb-1"><?php echo $oPlayer->height ?></h6>
					<h6 class="mb-1"><?php echo $oPlayer->weight ?></h6>
		...
		<div class="col-md-6">
			...
			<?php endif ?>
			<?php if (isset($user['username']) and !is_null($user['username'])): ?>
				<button id="editar" player_id="<?php echo $oPlayer->id_sofifa?>" class="btn btn-dark float-rigth">EDITAR</button>	
			<?php endif ?>
...
<script type="text/javascript">
	...
	// Btn Ver Jugador
	$('#sofifa').click(function () {
		var playe_id = $('#sofifa').attr('player_id');
		url = "<?php echo base_url('/jugador/sofifa/') ?>" + playe_id;
	...
	function getPosition(positions, clases = "") {
		...
		positions.forEach(function (position, index) {
			...
			// Delantera
			if ("CF★, CF, SS★, SS, WF★, WF".indexOf(position) >= 0) {
				var span = "<span class='badge badge-danger mr-1 "+clases+"'>"+position+"</span>";
			}

			// Medio campo
			if ("DMF★, DMF, WB★, WB, CMF★, CMF, SMF★, SMF, AMF★, AMF".indexOf(position) >= 0) {
				var span = "<span class='badge badge-success mr-1 "+clases+"'>"+position+"</span>";
			}

			// Defensa
			if ("CWP★, CWP, CB★, CB, SB★, SB".indexOf(position) >= 0) {
